      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, index) in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteShopCartById(cart)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      switch (type) {
        ...
        case "mins":
          ...
          //如果产品的个数小于等于1，传递给服务器个数0（原封不动）
          disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        case "change":
          //用户输入进来的最终的值，非法的（带有汉字），带给服务器的数字
  ...
  computed: {
    ...
    //全选按钮
    isAllCheck() {
      //遍历数组里面的原理，只要全部元素isChecked属性都为1==>真
      //只要有一个不是1==>假
